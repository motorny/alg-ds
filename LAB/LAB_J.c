...
#define _DELETE_FLAG_NOT_USED 0
#define _DELETE_FLAG_ACTIVE 1
#define _DELETE_FLAG_DELETED 2
...
HashTable_t* HashTableGet(uint32_t size, uint32_t mod)
{
  ...
  table->size = size;
  table->mod = mod;
  table->data = (HTItem*)malloc(sizeof(HTItem) * size);
  memset(table->data, _DELETE_FLAG_NOT_USED, sizeof(HTItem) * size);
  return table;
}
...
{
  return (x % n + x / n + x % k + x / k + (i * k) % n) % n;
}

static void HashTableRebuild(HashTable_t* table)
{
  uint32_t j, i, a;
  HTItem* oldData = table->data;
  table->data = (HTItem*)malloc(sizeof(HTItem) * table->size);
  memset(table->data, _DELETE_FLAG_NOT_USED, sizeof(HTItem) * table->size);
  for (j = 0; j < table->size; j++)
  {
    if (oldData[j].deleteFlag == _DELETE_FLAG_ACTIVE)
    {
      uint32_t strHash = oldData[j].strHash;
      for (i = 0; i < table->size; i++)
      {
        a = hashTFunc(table->size, i, table->mod, strHash);
        if (table->data[a].deleteFlag == _DELETE_FLAG_NOT_USED)
        {
          table->data[a].key = oldData[j].key;
          table->data[a].deleteFlag = _DELETE_FLAG_ACTIVE;
          table->data[a].strHash = strHash;
          table->data[a].value = oldData[j].value;
          break;;
        }
      }
    }
...
int HashTableFind(HashTable_t* table, uint32_t key, void** value_p)
{
  ...
  for (i = 0; i < table->size; i++)
  {
    ...
    else  if (deleteFlag == _DELETE_FLAG_DELETED)
    {
      delCount++;
    }
