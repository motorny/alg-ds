...
class FixHeight: public ::testing::Test {
protected:
    Node_t *root = nullptr;

    ~FixHeight() override { freeTree(root); }
...
TEST_F(FixHeight, 1Node) {
    root = insertNodes({1});
    ASSERT_EQ(root->len, 0);
}

TEST_F(FixHeight, Root1Left) {
    root = insertNodes({1, 0});
    ASSERT_EQ(root->left->len, 0);
    ASSERT_EQ(root->len, 1);
}

TEST_F(FixHeight, Root1Right) {
    root = insertNodes({0, 1});
    ASSERT_EQ(root->right->len, 0);
    ASSERT_EQ(root->len, 1);
}

TEST_F(FixHeight, Root1Left1Right) {
...
TEST_F(FixHeight, Root1Left2W1Right3W) {
...
TEST_F(FixHeight, Root2Left3W2W) {
...
TEST_F(FixHeight, Root2Right2W3W) {
