...
extern "C" {
#include "..\LabB\memallocator.h"
#include "..\LabB\Lab-B.cpp"
}
...
TEST(memfree, memfree_FreeValidPtr_ReturnSuccess) {
  ...
  char* ptr = (char*)memalloc(1);
  memfree(ptr);
  EXPECT_EQ(mSystem.descriptor1st->size, -1);
  EXPECT_EQ(mSystem.descriptor1st->next, (descriptor_t*)NULL);
  free(memory);
}
...
TEST(memfree, memfree_NeedToMergeNeighbourBlocks_SuccessfulMerging) {
  ...
  memfree(ptr3);
  memfree(ptr2);
  EXPECT_EQ(mSystem.descriptor1st->size, -(memgetminimumsize() + memgetblocksize() + 3)); //negative size means free memory
  EXPECT_EQ(mSystem.descriptor1st->next, (descriptor_t*)NULL);
  free(memory);
}
...
TEST(memfree, memfree_TryToFreeEmptyPointer_ReturnSuccess) {
  ...
  EXPECT_EQ(mSystem.descriptor1st->size, -1); //negative size means free memory
  EXPECT_EQ(mSystem.descriptor1st->next, (descriptor_t*)NULL);
}
