...
TEST(AddKey, EmptyTree_ReturnSuccess) {
  node_t* tree = NULL;

  ASSERT_EQ(AddKey(&tree, 1), B_SUCCESS);
  ASSERT_EQ(tree->num, 1);
  ASSERT_EQ(tree->isLeaf, TRUE);
  ASSERT_EQ(tree->keys[0], 1);
}

TEST(AddKey, FullNode_ReturnSuccess) {
  node_t* tree = NULL;
  AddKey(&tree, 1);
  AddKey(&tree, 2);
  AddKey(&tree, 3);
  ...
  ASSERT_EQ(AddKey(&tree, 6), B_SUCCESS);
...
TEST(FindNode, KeyIsInTree_ReturnPtr) {
  node_t* tree = NULL;
  AddKey(&tree, 1);

  ASSERT_NE(FindNode(tree, 1), nullptr);

  free(tree);
}

TEST(FindNode, KeyIsNotInTree_ReturnNull) {
	node_t* tree = NULL;
	AddKey(&tree, 1);

	ASSERT_EQ(FindNode(tree, 2), nullptr);

	free(tree);
}

TEST(RemoveKey, KeyIsInTree_ReturnSuccess) {
	node_t* tree = NULL;
	AddKey(&tree, 1);
	AddKey(&tree, 2);

	ASSERT_EQ(RemoveKey(&tree, 2), B_SUCCESS);
...
TEST(DestroyTree, NotEmptyTree_ReturnSuccess) {
	node_t* tree = NULL;
	AddKey(&tree, 1);
	AddKey(&tree, 2);
	AddKey(&tree, 3);

	EXPECT_EQ(DestroyTree(tree), B_SUCCESS);
}