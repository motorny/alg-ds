...
int Push(queue_t* queue, int data) {//add elem to queue
	if (!queue)
		return -1;
...
int Pop(queue_t* queue) {//take elem from queue
	int result;
	list_t* tmp;
...
graph_t* ReadGraph(char* str, int n, int len) {
