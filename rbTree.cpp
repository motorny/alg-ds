...
int findExponentK(struct rb_node* root, int expData) {
    if (root) {
        int rootValue = exponent(root->data);
        if (rootValue == expData) {
            return root->data;
        }
        else if (rootValue > expData) {
            return findExponentK(root->link[0], expData);
        }
        else {
            return findExponentK(root->link[1], expData);
        }
    }
    return ERROR;
}
int main(int argc, char* argv[]) {
    ...
    while (c != EOF)
    {
        ...
        case 'p': {
            int expData = exponent(data) - K;
            printf("%d\n", findExponentK(myTree, expData));
            break;
        }
        default: {
            printTree(myTree);
...
TEST(F21, addSomeValuesExpectCorrectTree) {
    ...
    add(&root, 100);
    add(&root, 1000);
    EXPECT_TRUE(findExponentK(root, 1) == 1);
    EXPECT_TRUE(findExponentK(root, 2) == 10);
    EXPECT_TRUE(findExponentK(root, 3) == 100);
    EXPECT_TRUE(findExponentK(root, 4) == 1000);
    freeTree(root);
}
TEST(F21, removeValueExpectCorrectTree) {
