#include "pch.h"
#include "../LabBMalloc.1/memallocator.c"

TEST(memInit, NULLpointer_ReturnMinus1)
{
	char* a = NULL;
...
TEST(memInit, NULLsize_ReturnMinus1)
{
	char a[100];
...
TEST(memInit, validArguments_Return1)
{
	char a[100];
	ASSERT_EQ(1, meminit(a, 100));
	memdone();
...
TEST(memInit, sizeLessMin_ReturnMinus1)
{
	char a[10];
...
TEST(memInit, reinitialization_RetutnMinus1)
{
	char a[20];
	char b[20];
	meminit(a, 20);
...
TEST(memalloc, NULLMinusBytes_ReturnNULL)
{
	char a[100];
	meminit(a, 100);
...
TEST(memalloc, PositiveBytes_ReturnValidPointer)
{
	char a[100];
	char* b, * c;
	meminit(a, 100);
