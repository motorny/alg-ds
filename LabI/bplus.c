...
Bplus* Init() {
	int i;
	Bplus* root = (Bplus*)malloc(sizeof(Bplus));
	if (root) {
		root->leaf = TRUE;
		root->key_count = 0;
		root->left = NULL;
...
void Split(Bplus** root, Bplus* node) {
	int i, key, pos;
	Bplus* newnode = malloc(sizeof(Bplus));
	newnode->leaf = FALSE;
	newnode->key_count = 0;
	newnode->left = NULL;
	newnode->right = NULL;
...
void Merge(Bplus** root, Bplus* node, int key) {
	Bplus* parent = node->parent;
	Bplus* right_bro = node->right;
	Bplus* left_bro = node->left;
	int i, pos = 0;
	Bplus* tmp = NULL;
